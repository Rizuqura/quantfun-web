...
(async () => {
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) {
      if (await evaluate('document.querySelectorAll("[data-asset]").length === 10')) break;
      await sleep(200);
    }
    await sleep(1000);
    ...
    assert.equal(await evaluate(`(()=>{const el=document.querySelector('[aria-label="Thesis cards. Swipe or use left and right arrow keys."]');return Math.abs(el.scrollLeft - (el.children[4].offsetLeft-el.children[0].offsetLeft)) < 3})()`), true);
    await screenshot('portfolio-btc');
    await evaluate('document.querySelector("[aria-label=\\"Next investment thesis\\"]").click()');
    await sleep(650);
    assert.equal(await evaluate('document.querySelector("[data-asset=MSTR]").getAttribute("aria-pressed")'), 'true');
    await evaluate('document.querySelector("[data-asset=TSLA]").focus()');
    await sleep(650);
    assert.equal(await evaluate('document.querySelector("[aria-label=\\"Next investment thesis\\"]").disabled'), true);
    ...
    await sleep(250);
    assert.equal(await evaluate('document.querySelector("[aria-label=\\"Choose an investment thesis\\"] [aria-pressed=true]").textContent'), 'COIN');
