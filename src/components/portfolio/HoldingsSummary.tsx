  ...
  return <section className={styles.holdings} aria-labelledby="holdings-title">
    ...
    <div className={styles.tableScroll} tabIndex={0} role="region" aria-label="Holdings table, scroll horizontally on smaller screens">
      <table className={styles.table}>
        ...
        <thead><tr><th scope="col">Asset</th><th scope="col">Bucket</th><th scope="col">Allocation</th><th scope="col">Value USD</th><th scope="col">Value IDR</th></tr></thead>
        <tbody>{positions.map((position) => {
          ...
          return <tr key={position.ticker} data-active={activeAsset === position.ticker} onPointerEnter={() => onActivate(position.ticker)} onPointerLeave={() => onActivate(null)} onClick={() => onActivate(position.ticker)}>
            ...
            <td className={styles.categoryCell}>{portfolioRoleLabels[position.portfolioRole]}</td><td>{formatAllocation(position.allocation)}</td><td>{formatUSD(position.valueUSD)}</td><td>{formatIDR(position.valueIDR)}</td>
          </tr>;
        })}</tbody>
        <tfoot><tr><th scope="row" colSpan={2}>Total portfolio</th><td>100%</td><td>{formatUSD(totals.valueUSD)}</td><td>{formatIDR(totals.valueIDR)}</td></tr></tfoot>
      </table>
    </div>
